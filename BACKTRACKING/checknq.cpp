...
bool solveNQueen(int board[N][N], int row) {
   ...
   for (int i = 0; i < N; i++) {     //for each row, check placing of queen is possible or not
      if (isValid(board, row, i) ) {
         board[row][i] = 1;      //if validate, place the queen at place (i, col)
         if ( solveNQueen(board, row + 1))    //Go for the other columns recursively
            return true;
                   
         board[row][i] = 0;        //When no place is vacant remove that queen
      }
   }
